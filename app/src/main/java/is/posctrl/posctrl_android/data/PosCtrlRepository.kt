    ...
    suspend fun sendReceiptInfoMessage(
        ...
        withContext(Dispatchers.Default) {
            try {
                ...
                val sendPacket = DatagramPacket(bytes, bytes.size, InetAddress.getByName(broadcastIp), port.toInt())
        ...
        withContext(Dispatchers.Default) {
            try {
                ...
                if (action == FilterAction.CLOSE) {
                    prefs.customPrefs()[appContext.getString(R.string.key_send_alife_filter)] = false
                }
        ...
        withContext(Dispatchers.Default) {
            try {
                ...
                while (true) {
                    ...
                    val sendAlife: Boolean = prefs.customPrefs()[appContext.getString(R.string.key_send_alife_filter)]
                            ?: true
                    if (!sendAlife) {
                        return@withContext
                    }
